...
# 1.1버전 업데이터 내역
# - 1. 통장임금 "거래일자_날짜" 추가 완료
# - 2. 검증결과 엑셀 다운로드시 Sheet 별 다운로드 개선완료

st.set_page_config(
    page_title="양산시 양곡관리 입금내역 매칭 서비스",
    page_icon="🏢",
)
...
st.title('양곡관리 입금내역 매칭 서비스 V1.1')
...
data = {
    '연번': [1,2,3],
    '구분': ['기초생계급여_기초의료급여','모자가족_부자가족_차상위계층_차상위본인부담경감대상자_차상위장애인_기초주거급여_기초교육급여','유형별 구분해서 입력해주세요'],
    '성명': ['제이홉', '진', '슈가',],
    '시군구': ['양산시', '양산시', '양산시',],
    '행정동': ['물금읍', '동면','원동면'],
...
file_uploader_label1 = '가. 양곡배부 대상자 명단을 등록하세요(Excel)'
...
if file2 is not None:
    try:
        ...
        # '거래일자' 컬럼의 데이터를 YYYY/MM/DD 형식으로 변환
        농협입금데이터['거래일자_날짜'] = pd.to_datetime(농협입금데이터['거래일자']).dt.date

        # '거래일자' 컬럼을 날짜와 시간이 모두 포함된 형식으로 변환
        농협입금데이터['거래일자_시간'] = pd.to_datetime(농협입금데이터['거래일자'])

        농협추출데이터 = 농협입금데이터[['거래일자_날짜','거래일자_시간', '입금금액(원)', '성명', '거래기록사항', '거래점']]
        #
        ...
        st.error(f"양곡 대상자 또는 농협 거래내역 파일이 등록되어 있지 않습니다.")
...
if '농협추출데이터' in locals():
   ...
   # 파일 머지 및 검증
    if file1 is not None and file2 is not None:
        ...
        정렬된_데이터프레임 = 결과[[
            '연번','거래일자_날짜','검증결과', '구분','성명','시군구','행정동','주소','세부주소','휴대전화번호','자택전화번호','양곡수량','생년월일','문자수신여부','가구원수(명)','본인부담금액(원)',
            '입금금액(원)','거래점','거래일자_시간','거래기록사항'
            ]]
...
# 엑셀 파일 생성
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        # 기초양곡 시트
        기초양곡_criteria = ['기초생계급여', '기초의료급여']
        정렬된_데이터프레임[정렬된_데이터프레임['구분'].isin(기초양곡_criteria)].to_excel(writer, index=False, sheet_name='기초양곡')

        # 차상위양곡 시트
        차상위양곡_criteria = ['모자가족', '부자가족', '차상위계층', '차상위본인부담경감대상자', '차상위장애인','기초주거급여','기초교육급여']
        정렬된_데이터프레임[정렬된_데이터프레임['구분'].isin(차상위양곡_criteria)].to_excel(writer, index=False, sheet_name='차상위양곡')

        # 확인필요 시트
        확인필요_criteria = 기초양곡_criteria + 차상위양곡_criteria
        정렬된_데이터프레임[~정렬된_데이터프레임['구분'].isin(확인필요_criteria)].to_excel(writer, index=False, sheet_name='확인필요')

    excel_data = output.getvalue()

    # 현재 날짜와 시간을 파일 이름에 포함
    current_datetime = datetime.now().strftime("%Y%m%d_%H%M%S")
    file_name = f"분류결과_{current_datetime}.xlsx"

    # 다운로드 버튼 생성
    st.download_button(
        label="📥 분류결과 다운로드(Excel)",
        data=excel_data,
        file_name=file_name,
